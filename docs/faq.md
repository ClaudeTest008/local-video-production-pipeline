...
**The pipeline "skipped" the video stage — why?**
ComfyUI wasn't reachable, or no video-capable workflow is enabled on the Workflows page. The run continues; fix and re-run — completed stages aren't repeated.
...
Yes — Ollama + ComfyUI are all local. Strategy then reasons without live web evidence and labels it accordingly. The `echo` provider even demos the pipeline with no AI installed.

**Where is my data? How do I back it up / reset?**
`%APPDATA%\LVPP Studio` (installed) or `backend/data` (dev). Copy the folder to back up; delete it to reset. Per-project archives: project page → "Export archive".
